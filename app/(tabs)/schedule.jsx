...
export default function ScheduleScreen() {
  ...
  const getCurrentDate = () => {
    ...
    return {
      ...
      dayName: now.toLocaleDateString('en-US', dayOptions)
    };
  };
  ...
  // Convert routes to schedule format based on selected day
  const getTodaySchedule = () => {
    if (!routes || routes.length === 0) {
      return [{}, {}, {}, {}]; // Empty slots
    }

    // Get the selected date to check the day of week
    const weekDates = getWeekDates();
    const selectedDateInfo = weekDates.find(day => day.date === selectedDay);
    ...
    // Filter routes based on frequency and day of week
    const filteredRoutes = routes.filter(route => {
      if (!route.frequency) return true; // Show if no frequency specified
      
      const dayOfWeek = selectedDateInfo.dayOfWeek; // 0 = Sunday, 1 = Monday, etc.
      
      // Check frequency patterns
      if (route.frequency.toLowerCase().includes('monday') && dayOfWeek === 1) return true;
      if (route.frequency.toLowerCase().includes('tuesday') && dayOfWeek === 2) return true;
      if (route.frequency.toLowerCase().includes('wednesday') && dayOfWeek === 3) return true;
      if (route.frequency.toLowerCase().includes('thursday') && dayOfWeek === 4) return true;
      if (route.frequency.toLowerCase().includes('friday') && dayOfWeek === 5) return true;
      if (route.frequency.toLowerCase().includes('saturday') && dayOfWeek === 6) return true;
      if (route.frequency.toLowerCase().includes('sunday') && dayOfWeek === 0) return true;
      
      // Check for "every day" or similar patterns
      if (route.frequency.toLowerCase().includes('every day') || 
          route.frequency.toLowerCase().includes('daily')) return true;
      
      // Check for "weekday" patterns
      if (route.frequency.toLowerCase().includes('weekday') && dayOfWeek >= 1 && dayOfWeek <= 5) return true;
      
      // Check for "weekend" patterns
      if (route.frequency.toLowerCase().includes('weekend') && (dayOfWeek === 0 || dayOfWeek === 6)) return true;
      
      return false;
    });

    // Convert filtered routes to schedule format
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.day}>Please wait</Text>
        </View>
  ...
  if (!collector) {
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          <Text style={styles.date}>Not Logged In</Text>
          <Text style={styles.day}>Please log in</Text>
        </View>
        <View style={styles.loadingContainer}>
          <Text style={styles.loadingText}>Please log in to view your schedule</Text>
        </View>
      </View>
    );
  }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.monthDisplay}>
        <Text style={styles.monthText}>
          {getWeekDates()[0].fullDate.toLocaleDateString('en-US', { 
            month: 'long', 
            year: 'numeric' 
          })}
        </Text>
                ...
                <ThemedView style={styles.card}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.cardTrashType}>
                        Type: <Text style={styles.trashType}>{item.icon} {item.trashType}</Text>
                      </Text>
                      {item.routeNumber && (
